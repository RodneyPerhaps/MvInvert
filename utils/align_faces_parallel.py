...
def get_landmark_from_image_array(img_array, predictor):
    ...
    for tt in t:
        a.append([tt.x, tt.y])
    lm = np.array(a, dtype=np.float32)
    return lm
...
def align_face(image_or_path, predictor):
    """
	:param filepath: str
	:return: PIL Image
	"""
    if isinstance(image_or_path, str):
        try:
            img = PIL.Image.open(image_or_path)
            if img.mode == 'RGBA':
                img = img.convert('RGB')
        except IOError:
        ...
        img = image_or_path.detach().cpu().numpy()
    else:
        img = image_or_path
