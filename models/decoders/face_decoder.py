...
class Face3D(nn.Module):
    def __init__(self, focal=1015, img_size=224, device='cuda:0', out_size=512):
        super(Face3D, self).__init__()
        self.facemodel = loadmat(model_paths['facemodel'])
        ...
        ckpt = torch.load(model_paths['restyle_encoder_early'], map_location='cpu')

        self.opts = ckpt['opts']

        self.opts['checkpoint_path'] = model_paths['restyle_encoder_early']
    ...
    def Reconstruction_Block(self, coeff, img_tensor=None, face_mask=None, is_train=True):
        ...
        #print(angles)
        vis_hat_ori, vis_hat, shape_dict = self.compute_vis_and_mesh_from_diff_angles(face_shape, face_shape_t, face_norm_r, self.translation)
        fitted_color = torch.clamp(face_color, 0, 255)
        ...
        samples_T['other'] = fitted_color
        samples_W['other'] = (1-face_vis)
        texcolor = self.Poisson_blending(shape_dict['origin'], self.tri.long()-1, samples_T, samples_W, POISSON_LAMBDA=0.1)


        return_tex, return_tex_color, imgs, tex_diff_poses, ginv = self.render_mesh_from_different_poses(vis_hat, shape_dict, texcolor, self.batch_size, img_tensor)
        completed_tex = self.render_uv(return_tex, self.batch_size)

        display_faces = self.display_from_diff_pose(face_shape, return_tex)

        return imgs, tex_diff_poses, ginv, display_faces, completed_tex, face_shape, self.tri - 1, return_tex_color / 255.0
    ...
    def multi_view_tex_completion(self, vis_hat, shape_dict, vtex, batch_num, img_tensor):
        tri = self.tri.long() - 1
        ...
        ori_visibility = self.Compute_vertex_vis(ori_shape2D, ori_zvertex, zbuffer, fbuffer, tri)
        samples_T['origin'], samples_W['origin'] = self.Sample_image(shape_dict['origin'], ori_shape2D, ori_shape2D, ori_visibility, img_tensor, fbuffer, 15.5)
        for k in ['right', 'left', 'front']:
            ...
            img_tensor = self.restyle_transform(img)
            with torch.no_grad():
            ...
            G[G>1] = 1
            G = G * 255
            samples_T[k], samples_W[k] = self.Sample_image(shape_dict['origin'], shape2D, aligned_shape2D, visibility, G, fbuffer, 1)
            

            _, T = self.extract_uv_with_img_and_shape2D(G, aligned_shape2D)
        ...
        return_color_v2 = self.Poisson_blending(shape_dict['origin'], tri, samples_T, samples_W, POISSON_LAMBDA=0.1)
    ...
    def compute_vis_and_mesh_from_diff_angles(self, face_shape, fitted_mesh, fitted_norm, translation=None):
        batch_num = face_shape.shape[0]

        angles =\
        torch.tensor([[ 0.0, -33 * np.pi / 180,  0.0],
                [ 0.0,  33 * np.pi / 180, 0.0],
                [ 0.0,  0.0, 0.0],
                [-12*np.pi/180, -33 * np.pi / 180,  0.0],
                [-12*np.pi/180,  33 * np.pi / 180,  0.0],
                [-12*np.pi/180, 0.0, 0.0]], dtype=torch.float32, device='cuda:0', requires_grad=False).unsqueeze(0)
        ...
        return vis_hat_ori, vis_hat, shape_t_dict
    ...
    def display_from_diff_pose(self, face_shape, com_tex):
        ...
        for k in ['right_60', 'left_60', 'front']:
            rotation = self.Compute_rotation_matrix(angles[k])
            face_shape_t = self.Rigid_transform_block(face_shape, rotation, translation)
        ...
        self.render_imgs = display_faces
