...
class ReconModel(nn.Module):
    ...
    def render_mesh_from_different_poses(self, vis_hat, shape_dict, vtex, batch_num):
        imgs_dict = {}
        shape2D_dict = {}
        tri = self.tri.long() - 1
        ...
        for k in ['right', 'left', 'front']:
            ...
            G = G * 255
            ginv[k] = G
            samples_T[k], samples_W[k] = self.Sample_image(shape_dict['front'], shape2D, aligned_shape2D, visibility, G, fbuffer, 2)

            tex, T = self.extract_uv_with_img_and_shape2D(G, aligned_shape2D)
            #samples_W[k] = vis_hat[k].unsqueeze(2)
            #samples_T[k] = T
        ...
        return_color_v2 = self.Poisson_blending(shape_dict['front'], tri, samples_T, samples_W, POISSON_LAMBDA=0.1)
    ...
    def Poisson_blending(self, face_shape, tri, samples_T, samples_W, POISSON_LAMBDA=0.1):
        ...
        for i in range(self.batch_size):
            ...
            A = vstack((GradMat[0], GradMat[1], GradMat[2], sparse.coo_matrix((cupy.fromDlpack(to_dlpack(POISSON_LAMBDA*(samples_W['ori'][i]>0)[:,0])), (cupy.arange(0, n), cupy.arange(0, n))), shape=(n,n))))
            y = cupy.vstack((y, POISSON_LAMBDA * cupy.fromDlpack(to_dlpack(samples_T['ori'][i]))))
            A_hat = A.T * A
    ...
    def forward(self, coeff, img_tensor=None):
        ...
        if img_tensor is not None:
            ...
            samples_W = {}
            samples_T['ori'] = vtex0
            samples_W['ori'] = vis_hat_ori.unsqueeze(2)
            samples_T['other'] = face_color
            samples_W['other'] = (~vis_hat_ori).unsqueeze(2)
            texcolor = self.Poisson_blending(shape_dict['front'], self.tri.long()-1, samples_T, samples_W, POISSON_LAMBDA=0.1)


            
            #return_tex, return_tex_color, imgs, tex_diff_poses, ginv = self.render_mesh_from_different_poses(vis_hat, shape_dict, vtex0, self.batch_size)
            return_tex = TexturesVertex(texcolor)
            completed_tex = self.render_uv(return_tex, self.batch_size)

            display_faces = self.display_from_diff_pose(face_shape, return_tex)
            vis_hat_ori = vis_hat_ori.float().repeat(3,1,1).permute(1,2,0)
            vis_tex = TexturesVertex(vis_hat_ori)
            vis_uv_img = self.render_uv(vis_tex, self.batch_size)
            inv_vis_tex = TexturesVertex(1- vis_hat_ori)
            inv_vis_uv_img = self.render_uv(inv_vis_tex, self.batch_size)
            ...
            return  display_faces, completed_tex, self.tri.long()-1, texcolor/255.0, face_shape, vis_uv_img, inv_vis_uv_img
