...
class ReconModel(nn.Module):
    ...
    def multiview_tex_completion(self, vis_hat, shape_dict, vtex, batch_num):
        ...
        samples_W['origin'] = self.Compute_weight(shape_dict['origin'], ori_shape2D, ori_zvertex, fbuffer, zbuffer, 12.5)
        samples_T['origin'] = T
    ...
    def Poisson_blending_torch(self, face_shape, tri, samples_T, samples_W, POISSON_LAMBDA=0.1):
        ...
        for i in range(self.batch_size):
            ...
            #dA = torch.sparse_coo_tensor(torch.stack((dA_cor, dA_cor)), dA_val, (m, m))
            GradMat = {}
            for j in range(0, 3):
            ...
            for k, v in samples_W.items():
                ...
                rhs[k] = SparseTensor.from_torch_sparse_coo_tensor(GradMatStack) @ samples_T[k][i]
            ...
            A_row, A_col, A_val = SparseTensor.from_torch_sparse_coo_tensor(torch.cat((GradMatStack, partA), 0)).coo()
            A_T = SparseTensor.from_torch_sparse_coo_tensor(torch.sparse_coo_tensor(torch.stack((A_col, A_row)), A_val, (n, 3*m+n)))
